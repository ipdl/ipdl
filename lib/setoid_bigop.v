...
(** https://github.com/coq-community/graph-theory/blob/master/theories/setoid_bigop *)
...
Lemma eqv_big_seq (I : eqType) (r:seq I) (P1 P2 : pred I) (F1 F2 : I -> X) :
  (forall x, x \in r -> P1 x = P2 x) -> (forall i, P1 i -> F1 i ≡ F2 i) ->
  \big[*%M/1]_(i <- r | P1 i) F1 i ≡ \big[*%M/1]_(i <- r | P2 i) F2 i.
Proof. 
  induction r.
  rewrite !big_nil //=.
